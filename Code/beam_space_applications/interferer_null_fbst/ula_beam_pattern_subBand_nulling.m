...
idx_2 = 175;
alpha = 0.5;
theta_i = asin(alpha*sin(Theta_fbst(idx_1)) + (1-alpha)*sin(Theta_fbst(idx_2)));
% idx_i = B;
% theta_i = Theta(idx_i);
...
for ii = 1:length(f_beam)
    for jj = 1:length(theta_beam)
        ...
        for nn=0:N-1
            ...
            a_theta_i = exp(-1i*2*pi*f_curr*tau_i);
            ym_nulled = ym - a_theta_i*a_theta_i'*ym/M;

            a_theta = exp(-1i*2*pi*f_curr*tau);
            Y_beam(nn+1) = a_theta'*ym_nulled;
