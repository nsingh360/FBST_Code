...
beta_lower_bound = (N-1)/N + sin(theta)/(2*fc)*(M-1)/(N*T);
beta_list = linspace(0.5*beta_lower_bound,1.5*beta_lower_bound,8); % oversampling for FE
L_list = 28:2:40;%ceil(beta_list/2 * N); % no. of samples in frequency is 2*L
beta_list = 2*L_list/N;
...
trials = 20;
SNR_dB = 30;
SNR_fe = zeros(length(L_list),trials);
SNR_fe_chirp = zeros(length(L_list),trials);
error_variance = zeros(length(L_list),trials);
error_variance_bound = zeros(length(L_list),trials);
error_variance_bound2 = zeros(length(L_list),trials);

error_bias = zeros(length(L_list),trials);
error_bias_bound = zeros(length(L_list),trials);
error_exact = zeros(length(L_list),trials);

for ii=1:length(L_list)
    L = L_list(ii);
    beta = beta_list(ii);
    %% define the fourier extension basis
    ...
    for jj=1:trials
        ...
        [alpha_true,error,error_bound] = fe_bias_error(@sig_gen, @fe_continuous, a, f, n_sinusoids, T_extended,fs_st, W_st, L);
        error_variance(ii,jj) = abs((alpha_true - alpha)'*Sigma*(alpha_true-alpha));
        error_variance_bound(ii,jj) = (2*L/N)*sigma^2/M;
        ...
        error_variance_bound2(ii,jj) = sum_1 + sigma^2/MN*sum_2;
...
%% plot beamformed SNR
figure(1)
plot(L_list,SNR_dB + db(M)/2,'LineWidth',1)
hold on
grid on
plot(L_list,db(mean(SNR_fe,2)),'-^','LineWidth',1)
hold on
grid on
plot(L_list, db(mean(SNR_fe_chirp,2)),'-^','LineWidth',1)
xlabel('FE dimension (L)','Interpreter','latex')
...
figure(2)
semilogy(L_list, mean(error_bias,2),'-o','LineWidth',1)
hold on
grid on
semilogy(L_list, mean(error_bias_bound,2),'-^','LineWidth',1)
xlabel('FE dimension (L)','Interpreter','latex')
...
figure(3)
semilogy(L_list, mean(error_variance,2),'-o','LineWidth',1)
hold on
grid on
semilogy(L_list, mean(error_variance_bound,2),'-^','LineWidth',1)
xlabel('FE dimension (L)','Interpreter','latex')
...
figure(4)
semilogy(L_list, mean(error_exact,2),'-o','LineWidth',1)
hold on
grid on
semilogy(L_list, mean(sqrt(2*(error_bias.^2 + 4*error_variance)),2),'-^','LineWidth',1)
hold on
grid on
semilogy(L_list, mean(sqrt(2*(error_bias_bound.^2 + 4*error_variance_bound)),2),'-^','LineWidth',1)
xlabel('FE dimension (L)','Interpreter','latex')
ylabel('Total error','Interpreter','latex')
legend({'error exact','error bound (Triangle Ineq.)', 'error bound (bias variance)'},'Location','northwest','Interpreter','latex')