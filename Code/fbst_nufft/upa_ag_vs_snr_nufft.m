...
j_idx = 3;%randsample(1:B+1,1);
k_idx = 21;%randsample(1:2*B+1,1);
...
for ii=1:trials
    for jj=1:length(SNRs)
        ...
        for ll=1:2*L+1
                ...
                X2_transform(:,:,ll) = nufft(squeeze(F_transform(:,:,ll)).',-m_nunif*(m_stoc + (sqrt(M)/2-1/2)),0:B,1).';%czt(F_transform(:,:,ll).',B+1,W_czt_stage2_coeffs(ll),1).';%g.';
        ...
        for ll=1:2*L+1
                ...
                nufft_coeff = A_coeff_stage3(ll).^(-( m_stoc + (sqrt(M)/2-1/2)));
                X2_transform_ = X2_transform(:,:,ll).*(nufft_coeff.');
                X1_transform(:,:,ll) = nufft(X2_transform_,-m_nunif*(m_stoc + (sqrt(M)/2-1/2)),0:(2*B),1);%czt(X2_transform(:,:,ll),2*B+1,W_czt_stage3_coeffs(ll),A_coeff_stage3(ll));%g;
