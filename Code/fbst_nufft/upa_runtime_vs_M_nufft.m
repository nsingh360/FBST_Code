...
for m_num = 1:length(M_lst)
    ...
    for jj=1:trials
        ...
        for ll=1:2*L+1
                ...
                X2_transform(:,:,ll) = nufft(squeeze(F_transform(:,:,ll)).',-m_nunif*(m + (sqrt(M)/2-1/2)),0:B,1).';
        ...
        for ll=1:2*L+1
                ...
                nufft_coeff = A_coeff_stage3(ll).^(-( m + (sqrt(M)/2-1/2)));
                X2_transform_ = X2_transform(:,:,ll).*(nufft_coeff.');
                X1_transform(:,:,ll) = nufft(X2_transform_,-m_nunif*(m + (sqrt(M)/2-1/2)),0:(2*B),1);
