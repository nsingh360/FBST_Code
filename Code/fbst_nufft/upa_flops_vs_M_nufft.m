...
for mm1 = 1:length(M_lst)
    M = M_lst(mm1); % sqrt(M) x sqrt(M) planar array
    ...
    chirpz_beam_domain1 = t_basis*sqrt(M)*chirpz_flops(sqrt(M),B+1);
    chirpz_beam_domain2 = t_basis*(B+1)*chirpz_flops(sqrt(M),2*B+1);
    ...
    canonical_vec_flops = t_beams*(2*fft_flops(2*t_basis,4*t_basis));

    %% Additional FLOPS for NUFFT interpolation
    window_len = 8; % window length of nufft
    sigma_fac = 2; % oversampling for nufft
    interp_flops = sigma_fac*window_len*( M*t_basis + sqrt(M)*(B+1)*t_basis ); % assuming accuracy level on the order of bias.

    %% FLOPS for FBST (reconstruction)
    reconstruc_flops = (t_beams)*chirpz_flops(t_basis,N);

    Runtime_fbst_precompute(mm1) = (chirpz_time_domain + chirpz_beam_domain1 + chirpz_beam_domain2 + precompute_flops + reconstruc_flops + interp_flops)/N;
    Runtime_fbst_superfast(mm1) = (chirpz_time_domain + chirpz_beam_domain1 + chirpz_beam_domain2 + canonical_vec_flops + reconstruc_flops + interp_flops)/N;

    % %% FLOPS for brute-force delay and sum
    % Runtime_ds_16_tap(mm1) = t_beams*M*16;
    % Runtime_ds_32_tap(mm1) = t_beams*M*32;

    %% FLOPS for fast delay and sum
    Nf = N + 2*16+1;
    Runtime_fast_ds_16_tap(mm1) = M*log2(M)*(30*Nf*log2(Nf)+12*Nf+2*N)/N;

    Nf2 = N + 2*32+1;
    Runtime_fast_ds_32_tap(mm1) = M*log2(M)*(30*Nf2*log2(Nf2)+12*Nf2+2*N)/N;

    %% FLOPS for sub-band processing (no zero padding)
    freq_fft_flops = M*fft_flops(N,N);
    chirpz_freq_beams_flops1 = N*sqrt(M)*chirpz_flops(sqrt(M),B+1);
    chirpz_freq_beams_flops2 = N*(B+1)*chirpz_flops(sqrt(M),2*B+1);
    time_fft_flops = t_beams*fft_flops(N,N);

    Runtime_DFT_beamformer(mm1) = (freq_fft_flops + chirpz_freq_beams_flops1 + chirpz_freq_beams_flops2 + time_fft_flops)/N;
    ...
    % 
    % Nf = N + 2*32+1;
    % Runtime_fast_ds_32_tap(mm1) = M*log2(M)*(30*Nf*log2(Nf)+12*Nf+8*N)/N;
...
xticklabels({'2^2','2^4','2^6','2^8','2^{10}', '2^{12}','2^{14}', '2^{16}','2^{18}','2^{20}'}); % Label as powers of 2
xlabel('Array size (M)','Interpreter','latex')
ylabel('Floating Point Operations (FLOPS)','Interpreter','latex')
legend({'FBST Superfast','FBST Precompute','Delay and Sum (R=16)', 'Delay and Sum (R=32)'},'Location','northwest','Interpreter','latex','FontSize',11)
exportgraphics(gcf, 'upa_flops_vs_M_nufft.pdf', 'ContentType', 'vector');
