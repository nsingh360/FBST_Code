...
for mm1 = 1:length(M_lst)
    ...
    interp_flops = sigma_fac*window_len*M*t_basis; % assuming accuracy level on the order of bias.

    Runtime_fbst_precompute(mm1) = (chirpz_time_domain + chirpz_beam_domain + precompute_flops + reconstruc_flops + interp_flops)/N;
    Runtime_fbst_superfast(mm1) = (chirpz_time_domain + chirpz_beam_domain + canonical_vec_flops + reconstruc_flops + interp_flops)/N;
...
exportgraphics(gcf, 'ula_flops_vs_M_nufft.pdf', 'ContentType', 'vector');
