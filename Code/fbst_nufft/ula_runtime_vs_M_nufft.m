...
for mm1 = 1:length(M_lst)
    ...
    theta = Theta(idx);
    % m_dev = randn(M,1);
    load(sprintf('runtime_ULA_nufft_data/m_dev_%d.mat',MN));
    % save(sprintf('runtime_ULA_nufft_data/m_dev_%d.mat',MN),'m_dev');
    ...
    % save(['runtime_ULA_nufft_data\' filename],'A_eplitz','toeplitz_inv');
    load(sprintf('runtime_ULA_nufft_data/A_eplitz_and_inv_%d.mat',MN));
    ...
    for jj = 1:trials
        ...
        for ll=1:2*L+1
            ...
            X_czt(ll,:) = nufft(F_transform_,-m_nunif*(x_pos+ lambda/2*(M/2-1/2))/(lambda/2),0:2*B);%czt(F_transform(:,ll),2*B+1,W_czt,A);
...
legend({'FBST Superfast','FBST Precompute','Delay and Sum (R=16)', 'Delay and Sum (R=32)'},'Location','northwest','Interpreter','latex','FontSize',12)
