...
lambda = c/(fc+exp_fac*Ws); % wavelngth
eta = lambda/(2*c);
W = 5e9; % bandwidth
...
Q = inv(A_eplitz + 1e-8*eye(2*L+1))*A_eplitz*inv(A_eplitz + 1e-8*eye(2*L+1));
...
for ii = 1:trials
    for jj = 1:length(SNRs)
        ...
        t_nufft = tic;
        m_n = x_pos/(lambda/2);
        m_transform = (M-1)/(max(m)-min(m))*(m-min(m));
        ...
        for ll=1:2*L+1
            l_dash = ll-1;
            m_nunif = 1/(2*B)*(fc_norm + (exp_fac/(2*fc_tilde*T*L))*(l_dash-L));
            ...
            F_transform_ = F_transform(:,ll).*(A.^(-(x_pos+ lambda/2*(M/2-1/2))/(lambda/2)));
            X_czt(ll,:) = nufft(F_transform_,-m_nunif*(x_pos+ lambda/2*(M/2-1/2))/(lambda/2),B_pts);%czt(F_transform(:,ll),2*B+1,W_czt,A);
            X_czt(ll,:) = coeff2*X_czt(ll,:).*coeff1;
        end
        w_l = X_czt(:,idx);
        %% beamforming inverse problem
        alpha = inv(A_eplitz + delta*eye(2*L+1))*w_l;
        alpha_ls = inv(A_eplitz + delta*eye(2*L+1))*F_basis'*y(:);
        y_nyq_ls = psi*alpha_ls;
        y_nyq = psi*alpha;
        run_nufft = run_nufft + toc(t_nufft);
        ...
        run_chirp = run_chirp + toc(t_chirp);
        ...
        % end
        % runtime_fbst0 = toc;
        % s_nyq_2 = slepian_sig_gen(V_nyq,a,n_sinusoids,t_nyq+1/fs);
        s_nyq_3 = slepian_sig_gen(V_nyq,a,n_sinusoids,t_nyq'+S/fs);
...
hold on
grid on
plot(SNRs,db(mean(SNR_fbst_2)),'-*','LineWidth',1)
hold on
grid on
...
legend({'Ideal','FBST NUFFT', 'FBST Chirp', 'TTD (R=16)', 'TTD (R=32)'},'Location','northwest','Interpreter','latex','FontSize',12)
