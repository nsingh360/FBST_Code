...
for ii = 1:trials
    for jj = 1:length(SNRs)
        ...
        % s_nyq_2 = slepian_sig_gen(a,f,n_sinusoids,t_nyq+1/fs);
...
hold on
grid on
plot(SNRs, db(mean(SNR_slepian_filt32)),'-^','LineWidth',1)
xlabel('Nominal SNR (dB)','Interpreter','latex','FontSize',12)
ylabel('Beamformed SNR (dB)','Interpreter','latex','FontSize',12)
legend({'Ideal','FBST (1.05W)', 'FBST (W)', 'Slepian filter (R=16)'},'Location','northwest','Interpreter','latex','FontSize',12)
