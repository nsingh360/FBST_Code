...
%% NOTE: Setting exp_fac>1 for NUFFT requires lambda=c/(fc+Omega+eta) for a small eta to avoid spatial aliasing.
...
m_dev = randn(M,1);
m = [(-M/2+1/2):(M/2-1/2)]' + m_dev/sqrt(lambda/2);
x_pos = m*lambda/2; % sensor positions
u_s = sin(theta); % normal vector for determining delays

% Sensor positions
x = x_pos;              % example sensor locations
y = ones(size(x_pos));         % constant height for stem markers
...
ax_inset = axes('Position',[0.55 0.55 0.3 0.3]); % [x y w h]
hold(ax_inset,'on')
...
xlim([0.1 0.2])                % zoom region
yticks([])
xticks([])
h.BaseLine.Visible = 'off'; 
axis(ax_inset,'off')
title('Zoomed in')
exportgraphics(gcf, 'lisa_non_unif_placement.pdf', 'ContentType', 'vector');
% figure;
% plot(x_pos, 0*ones(size(x_pos)), 'x','MarkerSize',10)
% hold on;
% plot([(-M/2+1/2):(M/2-1/2)]'*lambda/2, 0*ones(size(x_pos)), 'o')
...
for ii = 1:trials
    for jj = 1:length(SNRs)
        ...
        alpha_ls = toeplitz_inv*F_basis'*y(:);
        y_nyq_ls = psi*alpha_ls;
        ...
        s_nyq_2 = zeros(N,1);
        s_nyq_2(1:N-1) = s_nyq(2:end);

        SNR_ttd(ii,jj) = norm(s_nyq_2(comp_idxs))/norm(y_bf_ttd32(comp_idxs).' - s_nyq_2(comp_idxs));
        SNR_fbst(ii,jj) = norm(s_nyq)/norm(s_nyq - y_nyq_ls);
...
legend({'Ideal','FBST','Delay and Sum (R=32)'},'Location','northwest','Interpreter','latex','FontSize',12)
exportgraphics(gcf, 'lisa_array_gain_non_unif.pdf', 'ContentType', 'vector');


%% params for beam pattern plot
betas_no_null = inv(F_basis'*F_basis + 1e-5*eye(2*L+1))*(F_basis)';
...
F_interference = exp(1i*2*pi*t_i*freq_samples).*demod_phase_i;
% F_interference2 = F_interference2.*demod_phase_i2;
beta_null = inv(F_basis'*F_basis + 1e-8*eye(2*L+1))*F_basis'*(eye(MN,MN) - F_interference*inv(F_interference'*F_interference + 1e-8*eye(2*L+1))*F_interference');
W = 5e9; % bandwidth
f_beam = linspace(fc-0.5*W,fc+0.5*W,10);
...
for ii = 1:length(f_beam)
    for jj = 1:length(theta_beam)
        ...
        y = reshape(e_jj.*conj(demod_phase),[M,N,1]);
...
hold on
grid on
plot(rad2deg(theta_beam),db(beam_pattern_ds),'Color',[0 0 1 1]);
...
hold on
grid on
plot(rad2deg(theta_beam),db(beam_pattern_fbst),'Color',[0 0 1 1]);
xlabel('$\theta$ (degrees)','Interpreter','latex','FontSize',12)
ylabel('Response(dB)','Interpreter','latex','FontSize',12)
legend({'Distortionless response', 'FBST'},'Location','northwest')
exportgraphics(gcf, 'lisa_beam_pattern_non_unif.pdf', 'ContentType', 'vector');

figure(4)
plot(rad2deg(theta_beam),zeros(length(theta_beam),1),'r--');
hold on
grid on
plot(rad2deg(theta_beam),db(beam_pattern_null),'Color',[0 0 1 1]);
xlabel('$\theta$ (degrees)','Interpreter','latex','FontSize',12)
ylabel('Response(dB)','Interpreter','latex','FontSize',12)
legend({'Distortionless response', 'FBST - Subspace Nulling'},'Location','northwest')
exportgraphics(gcf, 'lisa_beam_pattern_nulling_non_unif.pdf', 'ContentType', 'vector');
