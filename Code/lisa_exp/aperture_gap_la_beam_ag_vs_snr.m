...
M_g = M/2; % divide array into two with one aperture gap
...
m_1 = [(-M_g/2+1/2):(M_g/2-1/2)]' - 3*M_g/2;
m_2 = [(-M_g/2+1/2):(M_g/2-1/2)]' + 3*M_g/2;
...
exportgraphics(gcf, 'lisa_array_gain_gap.pdf', 'ContentType', 'vector');
...
ax1 = axes('Position', [0.5, 0.7, 0.2, 0.2]); % [x, y, width, height]
box on;
plot(rad2deg(theta_beam(1640:1690)),zeros(length(theta_beam(1640:1690)),1),'r--');
hold on;
plot(rad2deg(theta_beam(1640:1690)), db(beam_pattern_null(:,1640:1690)), 'Color',[0 0 1 1], 'LineWidth', 1);
y1 = ylim;
ylim([y1(1), 1.5]);
exportgraphics(gcf, 'lisa_beam_pattern_gap.pdf', 'ContentType', 'vector');
...
ylim([y1(1), 1.5]);
set(gca, 'YTick', []);
% exportgraphics(gcf, 'lisa_beam_pattern_nulling_gap.pdf', 'ContentType', 'vector');
